...
export default function App() {
  ...
  function handleSearch(query) {
    ...
    const filtered = history.filter(h =>
      ...
      (h.data?.brief ? h.data.brief.toLowerCase().includes(query.toLowerCase()) : false)
    );
            ...
            >
              ...
              <div style={{ padding:"16px", borderBottom:`1px solid ${T.border}` }}>
                <input
                  ...
                  onFocus={e=>{ e.currentTarget.style.borderColor=T.accent; e.currentTarget.style.background=T.card; }}
              ...
              <div style={{ flex:1, overflowY:"auto", padding:"8px 10px" }}>
                {searchQuery.trim() === "" ? (
                  <div style={{ padding:"40px 20px", textAlign:"center", color:T.textMuted }}>
                    <div style={{ fontSize:12 }}>Start typing to search...</div>
                  </div>
                ) : searchResults.length === 0 ? (
                  <div style={{ padding:"40px 20px", textAlign:"center", color:T.textMuted }}>
                    <div style={{ fontSize:12 }}>No results found</div>
                  </div>
                ) : (
...
/* ═══════════════════════════════════════════════
   SIDEBAR
═══════════════════════════════════════════════ */
function Sidebar({ activeSection, setActiveSection, translatorHistory, teamHistory, activeChat, setActiveChat, onNewChat, onDelete, onPin, onRename, onShare, isOpen, isMobile, onShowSearch }) {
  const sections = [
    ...
    <aside style={{
      ...
      <div style={{ padding:"10px 10px", borderBottom:`1px solid ${T.border}` }}>
        <button
          ...
          onMouseEnter={e=>{ e.currentTarget.style.background=T.card; e.currentTarget.style.borderColor=T.accent; e.currentTarget.style.color=T.text; }}
          onMouseLeave={e=>{ e.currentTarget.style.background=T.surface; e.currentTarget.style.borderColor=T.border; e.currentTarget.style.color=T.textSoft; }}
        >
        ...
        {showUploadModal && (
          ...
          >
            <div
              style={{
                position:"fixed",
                bottom: isMobile ? "calc(100% - 100px)" : "auto",
                top: isMobile ? "auto" : "calc(100% - 120px)",
                right:"20px",
                background:T.card,
                border:`1px solid ${T.border}`,
                borderRadius:8,
